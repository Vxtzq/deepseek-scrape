...
app = FastAPI(title="DeepSeek God-Mode Bridge", version="5.8")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
MODEL_MAP = {
    ...
    "deepseek/generic": "DeepSeek-V4-Pro",
}
...
def _process_candidate(candidate, allowed_tools, result_list, seen_names):
    ...
    result_list.append({
        "index": len(result_list), "id": f"call_{uuid.uuid4().hex[:8]}", "type": "function",
...
async def chat_completions(request: Request, authorization: str = Header(None)):
    ...
    for cached_prompt, cached_response in recent_prompts:
        if cached_prompt == prompt:
            print(f"♻️ [API] Duplicate prompt detected, returning cached response", flush=True)
            if stream:
                ...
                return {"id": f"chatcmpl-{task_id}", "object": "chat.completion", "created": 1677652288, "model": requested_model, "choices": [{"index": 0, "message": {"role": "assistant", "content": cached_response}, "finish_reason": "stop"}]}
    ...
    command_queue.put({"action": "send_prompt", "prompt": prompt, "reasoning_mode": "Think"}) # FORCER THINK POUR LES TESTS
    ...
    if stream:
            ...
            try:
                ...
                while True:
                    ...
                    if token_data.get("type") == "content":
                        ...
                        if reasoning:
                            ...
                            yield f"data: {json.dumps({'id': f'chatcmpl-{task_id}', 'object': 'chat.completion.chunk', 'created': 1677652288, 'model': requested_model, 'choices': [{'index': 0, 'delta': {'reasoning_content': reasoning}, 'finish_reason': None}]})}\n\n"
                        ...
                        if name and not current_tool_call_id:
                            ...
                            tool_call_chunk = {"index": 0, "id": current_tool_call_id, "type": "function", "function": {"name": name, "arguments": args or ""}}
                        else:
                            tool_call_chunk = {"index": 0, "function": {"arguments": args}}
                ...
                if has_tools:
                    ...
                    if openai_tool_calls:
                        print(f"✅ [API] {len(openai_tool_calls)} tool call(s) detected.", flush=True)
        ...
        try:
            while True:
                ...
                if token_data.get("type") == "content":
                    ...
                    full_reasoning += token_data.get("reasoning", "")
        except asyncio.TimeoutError:
            print("⚠️ [API] Timeout 120s", flush=True)
        finally:
            async with response_lock:
        ...
        if has_tools:
            ...
            if openai_tool_calls:
                return {"id": f"chatcmpl-{task_id}", "object": "chat.completion", "created": 1677652288, "model": requested_model, "choices": [{"index": 0, "message": {"role": "assistant", "content": None, "tool_calls": openai_tool_calls}, "finish_reason": "tool_calls"}]}
        ...
        return {"id": f"chatcmpl-{task_id}", "object": "chat.completion", "created": 1677652288, "model": requested_model, "choices": [{"index": 0, "message": message, "finish_reason": "stop"}]}
...
async def receive_stream(request: Request):
    ...
    if data.get("type") == "stream":
        ...
        reasoning = data.get("reasoning", "") # ✅ RÉCUPÉRATION DU RAISONNEMENT
        
        if response_queue is not None:
            if content or reasoning:
                print(f"📝 [Server] Received content ({len(content)} chars) | reasoning ({len(reasoning)} chars)", flush=True)
                await response_queue.put({"type": "content", "text": content, "reasoning": reasoning})

    elif data.get("type") == "function_call":
        if response_queue is not None:
            await response_queue.put({"type": "function_call", "name": data.get("name", ""), "arguments": data.get("arguments", "")})
        ...
        print(f"❌ [API] BROWSER ERROR: {data.get('content')}", flush=True)
        if response_queue is not None:
            await response_queue.put({"type": "done"})
